...
def cal_index(root, base, total, models):
    ...
    for sdir in res_dirs:
        ...
        #cal ssim
        if flag_ssim:
            if 'ssim' not in index[sdir] or index[sdir]['ssim'] == np.nan:
                mssim[sdir] = []
...
if __name__ == '__main__':
    ...
    if os.path.exists('index_whole.npy'):
        total = np.load('index_whole.npy', allow_pickle=True).item()
    ...
    for item in root_dirs:
        ...
        # if "refine-G" not in item : continue
        total[item] = cal_index(root, item, total, models)

        np.save('index_whole.npy',total)
