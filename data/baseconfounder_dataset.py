...
def get_transform_params(full_size, inst_info=None, class_of_interest=None, 
    ...
    if bbox == None:
      ...
      crop_poss, crop_objects, bbox_in_contexts, bbox_clss, bbox_inst_ids  = \
    ...
    output_dicts = []
    for crop_pos, crop_object, bbox_in_context, bbox_cls, bbox_inst_id in zip(crop_poss, crop_objects, bbox_in_contexts, bbox_clss, bbox_inst_ids ):
        output_dict = {
            'crop_pos': crop_pos,
            'flip': flip,
...
def crop_single_object(inst_info, class_of_interest, 
    ...
    candidate_list = sample_fg_from_full(inst_info, class_of_interest, min_box_size, test_addition)
    # sample random box
    sample_bg = random.random() < prob_bg
    if sample_bg or (candidate_list is None):
        if label_info is None or class_of_background is None:
            candidate_list = sample_bg_from_full(min_box_size, max_box_size, w, h) 
        else:
            candidate_list = sample_bg_from_labels(min_box_size, max_box_size, w, h, label_info, class_of_background) 
    
    crop_poss, crop_objects, bbox_in_contexts, bbox_clss, bbox_inst_ids = [], [], [], [], []
    for bbox_selected in candidate_list:
        ...
        crop_pos = crop_box_with_margin(bbox_selected['bbox'], w, h, \
            img_to_obj_ratio, random_crop)#较大裁剪区域的bbox信息，含随机性，代表训练过程中256x256的完整输入图
        crop_object =  crop_box_with_margin(bbox_selected['bbox'], w, h, \
            patch_to_obj_ratio, random_crop)#较小裁剪区域，即object的bbox信息，含随机性
        bbox_in_context = get_bbox_in_context(bbox_selected, crop_pos, target_size)#object在crop_pos中的相对坐标，[0,256]
        if flip:
        ...
        crop_poss.append(crop_pos) 
        crop_objects.append(crop_object) 
        bbox_in_contexts.append(bbox_in_context) 
        bbox_clss.append(bbox_cls) 
        bbox_inst_ids.append(bbox_inst_id)
    return crop_poss, crop_objects, bbox_in_contexts, bbox_clss, bbox_inst_ids 
...
def sample_bg_from_labels(min_box_size, max_box_size, w, h, label_info, class_of_background):
...
#   label_info = Image.open(label_info)
  label = np.array(label_info)

  mask = np.where(np.isin(label,[6, 8, 9]))
  if not len(mask[0]):
    mask = np.where(label==7)
  i = np.random.randint(0, len(mask[0])-1)
  y,x = mask[0][i], mask[1][i]
  x_size = np.random.randint(min_box_size, max_box_size)
  y_size = np.random.randint(min_box_size, max_box_size)
  ymin = np.random.randint(0, h-min_box_size-1)
  xmin, xmax = max(0, x-x_size//2), min(x+x_size//2,w-1)
  ymin, ymax = max(0, y-y_size//2), min(y+y_size//2,h-1)
  bbox_selected = {'bbox': [xmin, ymin, xmax, ymax], 'cls': None, 'inst_id': None}

  del label_info, label, mask  
  del class_of_background
  assert xmin<xmax
  assert ymin<ymax
