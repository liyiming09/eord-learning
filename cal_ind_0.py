    ...
    for sdir in res_dirs:
        ...
        if flag_fid:
            if 'fid' not in index[sdir]:
                ...
                print(sdir, fid_value)
                index[sdir]['fid'] = fid_value
        #cal ssim
        if flag_ssim:
            if 'ssim' not in index[sdir]:
                mssim[sdir] = []
            ...
            #         print(images[i*6], psnr,ssim)
                ...
                print(sdir,np.mean(mssim[sdir]) )#np.mean(mpsnr[sdir]),
...
if __name__ == '__main__':
    ...
    for item in root_dirs:
        total[item] = cal_index(root, item, total)
