...
class AttentionEffectModel(torch.nn.Module):
    ...
    def compute_generator_loss(self, input_semantics, real_image, masked_image):
        ...
        if not self.eord_flag:
            if self.opt.divco:
                G_losses['Byol'] = self.criterionBYol(feat) \
                    * self.opt.lambda_divco
            if self.opt.effect:
                #效应计算公式：base-neg = (干预前后差别区域，real)
                # print(pred_neg[0][0].shape)
                # pred_effect = -pred_neg[0][0] + pred_fake[0][0][0:1,...]
                effect_map = self.invent_semantics[2:,...] - self.invent_semantics[0:1,...]
                effect_map[effect_map!=0] = 1#赋值为float32类型

                G_losses['Effect'] = self.criterionEffect(pred_neg, pred_fake,effect_map,  True, for_discriminator=False)* self.opt.lambda_effect
    ...
    def discriminate(self, input_semantics, fake_image, real_image, enc_feat = False):
        ...
        if not enc_feat:
            if self.opt.effect:
                ...
                pred_fake, pred_real, pred_neg = self.divide_pred(discriminator_out)
                return pred_fake, pred_real, pred_neg
            else:
                discriminator_out = self.netD(fake_and_real)
                pred_fake, pred_real, _ = self.divide_pred(discriminator_out)
                return pred_fake, pred_real
        else:
            # print(real_concat.shape,real_concat.shape, fake_and_real.shape)
            discriminator_out, feat = self.netD(fake_and_real, enc_feat)
            # print(3,discriminator_out[0][0].shape,discriminator_out[0][2].shape)
            pred_fake, pred_real, pred_neg = self.divide_pred(discriminator_out)
            # print(pred_fake[0][0].shape)
            return pred_fake, pred_real, pred_neg, feat
    ...
    def divide_pred(self, pred):
        ...
        if self.opt.effect:#区别主要在于是否返回neg部分
            if type(pred) == list:
                fake = []
                real = []
                neg = []
                for p in pred:
                    ...
                    if not self.eord_flag:
                        ...
                        neg.append([tensor[2:tensor.size(0) // 2] for tensor in p])
